...
func _ready() -> void:
	...
	_head = Node3D.new()
	_head.position.z = -0.55
	_body.add_child(_head)
	Geometry.sphere(_head,"Head",Vector3(0,0,-0.35),Vector3(0.41,0.46,0.65),silver)
	# Fixed-size dark opening; the small jaw lowers only when actually biting.
	_mouth = Geometry.sphere(_head, "Mouth", Vector3(0, -0.04, -0.99), Vector3(0.13, 0.085, 0.018), Geometry.material("08252e"))
	_mouth.visible = false
	_jaw = Geometry.sphere(_head, "Jaw", Vector3(0, -0.13, -0.90), Vector3(0.15, 0.06, 0.13), silver)
	for side in [-1, 1]:
		Geometry.sphere(_head, "Eye", Vector3(side * 0.31, 0.12, -0.60), Vector3.ONE * 0.085, Geometry.material("112e3b"))
		var fin = Node3D.new()
...
func _process(delta: float) -> void:
	...
	_head.rotation = Vector3(head_offset.x,head_offset.y,0)
