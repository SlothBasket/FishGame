...
func _ready() -> void:
	...
	Geometry.sphere(_body, "BodyForm", Vector3.ZERO, Vector3(0.46, 0.58, 1.25), silver)
	...
	_mouth = Geometry.sphere(_body, "Mouth", Vector3(0, -0.04, -1.235), Vector3(0.13, 0.085, 0.018), Geometry.material("08252e"))
	_mouth.visible = false
	_jaw = Geometry.sphere(_body, "Jaw", Vector3(0, -0.13, -1.13), Vector3(0.15, 0.06, 0.13), silver)
	for side in [-1, 1]:
		Geometry.sphere(_body, "Eye", Vector3(side * 0.31, 0.12, -0.85), Vector3.ONE * 0.085, Geometry.material("112e3b"))
		var fin = Node3D.new()
