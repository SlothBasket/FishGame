...
@export var zone_population: int = 8
...
@export var pod_population: int = 10
...
func _ready() -> void:
	...
	for center in [Vector3(-28,15,-20), Vector3(24,20,-28), Vector3(-20,22,32), Vector3(35,13,24), Vector3(-45,28,40), Vector3(38,28,-45), Vector3(-35,4,-45), Vector3(40,4,40)]:
		...
		for i in range(pod_population):
			...
			_spawn(BaitMotion.Kind.MINNOW, center + slot, _rng.randi(), 40, pod, slot)
	for center in [Vector3(-22,31.5,-18), Vector3(35,31.5,20)]:
		var pod = BaitPod.new(center)
		pods.append(pod)
		for i in range(8):
			var slot = Vector3(cos(i*TAU/8)*5, 0, sin(i*TAU/8)*5)
			_spawn(BaitMotion.Kind.MULLET, center+slot, _rng.randi(), 50, pod, slot)
	for i in range(midwater_squid_count):
		_spawn(BaitMotion.Kind.SQUID, Vector3(_rng.randf_range(-65,65), _rng.randf_range(14,24), _rng.randf_range(-65,65)), _rng.randi(), 45)
	for i in range(12):
	...
	for i in range(4):
		_spawn(BaitMotion.Kind.GULL, Vector3(-18 + i * 12, water_depth + 10, -12), _rng.randi(), 60.0)
...
func _spawn(kind: int, home: Vector3, behavior_seed: int, radius: float, pod: BaitPod = null, slot: Vector3 = Vector3.ZERO) -> BaitActor:
