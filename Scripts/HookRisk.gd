...
func step(delta: float, slack: float, shake: float, airborne: bool, severity: float, lowered: bool) -> void:
	...
	hazard = slack_base_hazard*slack_factor*looseness*(1+slack_shake_multiplier*shake)
	if airborne:
		hazard += airborne_base_hazard*clampf(severity,0,2)*looseness*(1+airborne_shake_multiplier*shake)*(0.4 if lowered else 1.0)
