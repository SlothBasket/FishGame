...
`BaitSchool` creates 142 actors by default: eight zones times `zone_population` 6, 12 upper-water bait, four independent mullet, four gulls, four surface entrants, eight minnow pods times `pod_population` 6, two mullet pods of eight, and `midwater_squid_count` 6. The minnow centers in `_ready()` span Y=4, 13-22 and 28 m; mullet pods are at 31.5 m. Edit those centers to move schools, or the population settings to increase density. Additional squid start at 14-24 m and all squid spawn heights are clamped to 14-25 m. Zone coordinates are fractions of arena width/depth; `individual_spacing` is 7 and `roam_radius` is 45.

`_initial_queue` stores spawn descriptions rather than nodes. A seeded shuffle mixes species; `_spawn_clock` creates one actor each `initial_spawn_interval` (0.10 s), taking about 15 s to fill the arena. Initial shrimp/crabs have random heights between bottom home and surface; replacements still arrive at the surface. Initial creation and respawns share a one-actor-per-frame limit, with initial fill taking priority. `seed_value = -1` varies play; set a nonnegative value for repeatable spawning. `_rng` belongs to the school; each driver gets its own seed. `_respawns` stores species, home, radius and any pod/slot assignment. `respawn_delay` is 8 seconds and at most one replacement is built per render frame. Pod membership is preserved, and dead weak references are pruned when replacements join.
