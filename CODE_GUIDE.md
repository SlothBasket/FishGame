...
The default arrays in `BaitActor._ready()`, `hit_radius()` and `nutrition()` follow `Kind` order. `body_size` scales both visible bait and its collision/bite radius. `randomize_size(rng)` runs before adding spawned AI or player bait: all species use the earlier base 0.8, multiplied by a uniform 0.8-1.2 (actual scale 0.64-0.96). Food values do not vary with size. Tests may set exact sizes directly. Gull flight uses `Seagull` rather than the generic underwater motor.
...
`BaitSchool` creates 226 actors by default: eight zones times `zone_population` 10, 12 upper-water bait, four independent mullet, four gulls, four surface entrants, eight minnow pods times `pod_population` 12, two mullet pods of eight, and `midwater_squid_count` 10. The minnow centers in `_ready()` span Y=4, 13-22 and 28 m; mullet pods are at 31.5 m. Edit those centers to move schools, or the population settings to increase density. Additional squid start at 14-24 m and all squid spawn heights are clamped to 14-25 m. Zone coordinates are fractions of arena width/depth; `individual_spacing` is 7 and `roam_radius` is 45.

`_initial_queue` stores spawn descriptions rather than nodes. A seeded shuffle mixes species; `_spawn_clock` creates one actor each `initial_spawn_interval` (0.10 s), taking about 23 s to fill the arena. Initial shrimp/crabs have random heights between bottom home and surface; replacements still arrive at the surface. Initial creation and respawns share a one-actor-per-frame limit, with initial fill taking priority. `seed_value = -1` varies play; set a nonnegative value for repeatable spawning. `_rng` belongs to the school; each driver gets its own seed. `_respawns` stores species, home, radius and any pod/slot assignment. `respawn_delay` is 8 seconds and at most one replacement is built per render frame. Pod membership is preserved, and dead weak references are pruned when replacements join.
...
The boat begins near an edge facing inward, but casts are no longer forced toward the center. A cast near a wall is shortened along the chosen ray to stay 10 m inside the arena edge. Boat movement and AI return checks use a 15 m edge margin. `arena_half_width` is passed from Reef through the school and controller into actors; this pass uses half-width 132 m. `anchor_position` and boat position share one origin. `spawn_position` records the landing/reset point. `deployment_position()` puts squid directly below the boat before it can roam its 8.4 m radius.
